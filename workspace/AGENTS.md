...
| skill | 何时用 |
|-------|--------|
| `understanding` | 高阶理解入口。开工默认先读，用于应用全局执行原则、澄清证据边界和问题本质；概念问题可由本层直接负责，具体化后再路由，命中语义主题时按需读取对应 reference。 |
...
接到需求先把当前 unresolved step 所需的信息核对清楚。
不要为纯资料查询、源码阅读、文档维护或本地编译分析强行补板级物理状态。
...
只有任务会触碰真实板子时才必须补齐这一组。
包括烧写、`bcu`、`uuu`、串口、boot mode、复位、启动、登录、网络验证等动作。
...
当前 step 的必要信息不齐，就停在这一步补齐，不往下走。
非当前 step 必需的信息，不要提前卡住任务。
...
每个 case 一个目录。这个目录要尽量保留完整过程，推荐最小结构是：

- `README.md`：目标、输入版本、成功判据、关键步骤、结论、交付项索引
- `records/`：过程记录、步骤笔记、命令线索
- `logs/`：串口日志、命令输出、运行日志、原始抓取
...
读 `understanding` 入口页时，
先应用其中的全局执行原则，再判断当前问题本质是否已经分清。
不等于把 `understanding/references/` 下所有主题文档一次性放进上下文。

- 如果问题本质仍未分清，由 `understanding` 继续负责
- 如果命中某个语义主题，按需读取对应 reference
- 如果问题已经具体化，再按当前 unresolved step 转到
  `support`、`compile` 或 `board-exec`

默认顺序：

- 先读 `understanding`
  应用全局原则，明确当前观察的证据边界和问题本质；
  必要时由本层直接完成概念分析
