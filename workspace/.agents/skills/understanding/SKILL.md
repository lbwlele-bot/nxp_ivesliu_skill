...
从研究转入实施前，如果存在多个合理方向，
而下一步会决定主要排查路线、跨多个模块、改变 release demo 逻辑
或显著影响后续实验解释，先与工程师对齐方向再动手。

客户描述、历史案例、工程师倾向和 AI 推断都是证据或先验，
不能把其中任何一个直接当成已证实根因。
对齐时至少讲清：

- 当前有哪些主要方向，各自依据和不确定性是什么
- 推荐先做哪个最小区分实验，以及为什么
- 该实验能够证明什么，不能证明什么

如果 plan 没有显式展示这些重大方向分歧，
不能把整体 plan 获批视为已经完成方向对齐。
工程师确认方向后再进入代码修改或大规模构建。
这条规则只用于方向性决策，
不把普通只读检查和低风险单变量验证变成逐项审批。
