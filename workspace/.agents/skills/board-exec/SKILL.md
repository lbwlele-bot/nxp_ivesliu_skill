...
当前 i.MX8DXL 的 manual-reset-only 是板型特例，
不能套到 i.MX93、i.MX943 或其它板；这些板继续按各自板级知识和现场确认
执行。
...
- 具体串口 role 事实仍读 `board_knowledge/<board>/serial.yaml`
...
- 每块板自己的 `serial.yaml`
  负责给 `serial-console` 消费的机器可读串口 role 映射
