...
- `i.MX95` 不是简单的“整板只跑一套 Linux”
- 当前 case 里，某些 `A` 核可能会被单独拿去跑 `Zephyr` 或 `FreeRTOS`
- 所以讨论 `A` 核 `RTOS` 时，先显式说清：
  目标 `A` 核是谁
  当前 payload 是什么
  预计由 `flash.bin`、`U-Boot` 还是 Linux 侧接手
- 在 `CPU0` 还停留在下载态、`FB` 或 `U-Boot` 阶段判断不清时，
  不提前宣称某个 `A` 核 `RTOS` 已经具备启动前提
...
- 如果当前阶段判断不清，先回头确认：
  ...
  当前 `uuu` 看到的下载态是什么
  `bcu` 是否留下了额外控制状态
- 在这块板上，`bcu` 可以参与恢复到可控态，
  但 `-keep` 留下的 GPIO / UART-mux 副作用必须一起回收
- 如果第一路运行态串口异常安静，
  优先先排查 `ft_fta_sel` 是否还被留在错误状态，
  不要直接把“假静音”误读成 Linux 或 `A` 核 `RTOS` 没起来
